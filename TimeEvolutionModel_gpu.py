...
class DissipationModel_GPU:
    ...
    def __init__(
        ...
        self.psi = cupy.zeros([N, N], cupy.complex128)
    ...
    def update(self, t_start, t_end, dt=1e-3):
        ...
        x = cupy.linspace(-self.L, self.L, self.N, dtype=cupy.complex128)
        y = cupy.linspace(-self.L, self.L, self.N, dtype=cupy.complex128)
        mx, my = cupy.meshgrid(x, y)

        ux = 2*cupy.pi*cupy.fft.fftfreq(self.N, d=1.0/self.N)/(2 * self.L)
        mux, muy = cupy.meshgrid(ux, ux)

        Ux = cupy.exp(1/(1j-r)*dt*(mux**2 + 2*w0*my*mux) /
                      4).astype(cupy.complex128)
        Uy = cupy.exp(1/(1j-r)*dt*(muy**2 - 2*w0*mx*muy) /
                      4).astype(cupy.complex128)
        Cd = 2*cupy.pi
        ...
        while t <= t_end:
            ...
            temp_psi = cupy.fft.fft(self.psi, axis=1)
            temp2_psi = cupy.multiply(temp_psi, Ux)
            self.psi = cupy.fft.ifft(temp2_psi, axis=1)

            temp_psi = cupy.fft.fft(self.psi, axis=0)
            temp2_psi = cupy.multiply(temp_psi, Uy)
            self.psi = cupy.fft.ifft(temp2_psi, axis=0)

            self.psi = cupy.exp(1/(1j-r)*dt*(((1 + ex)*mx**2 + (1 + ey) * my**2)/2
                                             + k*cupy.abs(self.psi)**2 - U)) * self.psi
